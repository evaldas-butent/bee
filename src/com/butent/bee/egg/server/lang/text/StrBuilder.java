 ...
 */
package com.butent.bee.egg.server.lang.text;
...
import com.butent.bee.egg.server.lang.ArrayUtils;
import com.butent.bee.egg.server.lang.SystemUtils;
...
public class StrBuilder implements CharSequence, Appendable {
   ...
   * do that, call <code>clear()</code> followed by {@link #minimizeCapacity()}.
   * <p>
   ...
   * @param endIndex
   *          the end index, exclusive, must be valid except that if too large
   *          it is treated as end of string
   * @return a new array that holds part of the contents of the builder
   * @throws IndexOutOfBoundsException
   *           if startIndex is invalid, or if endIndex is invalid (but endIndex
   *           greater than size is valid)
   */
  ...
  /**
   * Appends a iterable placing separators between each value, but not before
   * the first or after the last. Appending a null iterable will have no effect.
   ...
   * the first or after the last. Appending a null iterator will have no effect.
   * Each object is appended using {@link #append(Object)}.
   * 
   * @param it
   *          the iterator to append
   ...
   * <code>toString</code> of the object is used. If the object is larger than
   * the length, the left hand side is lost. If the object is null, the null
   * text value is used.
   * 
   * @param obj
  ...
  /**
   * Appends an object to the builder padding on the left to a fixed width. The
   * <code>String.valueOf</code> of the <code>int</code> value is used. If the
   * formatted value is larger than the length, the left hand side is lost.
   * 
   * @param value
   *          the value to append
   * @param width
   ...
   * you could write a matcher to delete all occurances where the character 'a'
   ...
   * <p>
   * Matchers can be used to perform advanced deletion behaviour. For example
   * you could write a matcher to delete where the character 'a' is followed by
   * a number.
   * 
   * @param matcher
   *          the matcher to use to find the deletion, null causes no action
  ...
  private void replaceImpl(int startIndex, int endIndex, int removeLen,
    ...
    if (insertLen != removeLen) {
      ensureCapacity(newSize);
      System.arraycopy(buffer, endIndex, buffer, startIndex + insertLen, size
          - endIndex);
      size = newSize;
    }
    if (insertLen > 0) {
   ...
   * <p>
   * Matchers can be used to perform advanced replace behaviour. For example you
   * could write a matcher to replace all occurances where the character 'a' is
  ...
  /**
   * Extracts the rightmost characters from the string builder without throwing
   * an exception.
   * <p>
   ...
   * <p>
   * This method extracts <code>length</code> characters from the builder at the
   * specified index. If the index is negative it is treated as zero. If the
   * index is greater than the builder size, it is treated as the builder size.
   * If the length is negative, the empty string is returned. If insufficient
   * characters are available in the builder, as much as possible is returned.
   ...
   * the buider and tokenizer within certain limits, however there is no
   ...
   * <p>
   * To use, simply create a <code>StrBuilder</code>, call <code>asWriter</code>
   * , and populate away. The data is available at any time using the methods of
   * the <code>StrBuilder</code>.
   * <p>
   * The internal character array is shared between the builder and the writer.
   * This allows you to intermix calls that append to the builder and write
   * using the writer and the changes will be occur correctly. Note however,
   * that no synchronization occurs, so you must perform all operations with the

package com.butent.bee.server.modules.orders.ec;
...
@WebServlet(urlPatterns = "/ord/*")
@SuppressWarnings("serial")
public class OrdersEcServlet extends LoginServlet {
  ...
  private static final String REG_STYLE_VAT_PREFIX = REG_STYLE_PREFIX + "vat-payer-";
  ...
  private Node companyTypeSelector(String label) {
    String name = COL_REGISTRATION_COMPANY_TYPE;
    Select select = select().name(name).id(name + ID_SUFFIX_INPUT).required().append(option());

    BeeRowSet types = qs.getViewData(ClassifierConstants.VIEW_COMPANY_TYPES);
    if (!DataUtils.isEmpty(types)) {
      int index = types.getColumnIndex(ClassifierConstants.COL_COMPANY_TYPE_NAME);

      for (BeeRow row : types.getRows()) {
  ...
  private static Node vatPayerSelector(Dictionary constants) {
    String name = COL_REGISTRATION_NOT_VAT_PAYER;
    ...
    container.append(label().addClass(REG_STYLE_VAT_PREFIX + "label").append(input, span));

    return tr().id(name + ID_SUFFIX_FIELD).append(td(),
        td().addClass(REG_STYLE_INPUT_CELL).append(container));
  }
  ...
  protected void doService(HttpServletRequest req, HttpServletResponse resp) {
    ...
    if (BeeUtils.isEmpty(path)) {
      html = getInitialPage(req, UserInterface.ORDERS);
      ...
      if (parameters.containsKey(COL_REGISTRATION_EMAIL)) {
  ...
  private String getRegistrationForm(String contextPath, Dictionary constants) {
    ...
    Tbody fields = tbody().append(
        ...
        companyTypeSelector(constants.companyStatus()),
  ...
  private String register(HttpServletRequest req, Map<String, String> parameters,
    ...
    SqlInsert si = new SqlInsert(VIEW_ORD_EC_REGISTRATIONS);

    for (Map.Entry<String, String> entry : parameters.entrySet()) {
      if (!BeeUtils.isEmpty(entry.getValue())
          && sys.hasField(VIEW_ORD_EC_REGISTRATIONS, entry.getKey())) {
    ...
    if (response.hasErrors()) {
      ...
      BeeRowSet data = qs.getViewData(VIEW_ORD_EC_REGISTRATIONS,
      ...
      for (int i = 0; i < data.getNumberOfColumns(); i++) {
        ...
        switch (column.getId()) {
          ...
          case COL_REGISTRATION_COMPANY_TYPE:

...
public class GridUtils {
  ...
  public static Element createGhostCell(Element el) {
    Element td = (el == null) ? DOM.createTD() : el;
  ...
  public static int distributeWidth(List<ColumnWidth> columns, int width) {
    ...
    return distributeOrderedWidth(orderedColumns, w);
  }
  ...
  private static int distributeOrderedWidth(List<ColumnWidth> columns, int width) {
    ...
    while (!columns.isEmpty() && w != 0) {
      ...
      if (totalRequired != 0) {
        diffAvailable = Math.min(1.0, w / (double) totalRequired);
      }
      for (int curIndex = 0; curIndex < syncedColumns; curIndex++) {
        ColumnWidth curInfo = columns.get(curIndex);
        ...
        if (growing && required > w) {
          required = w;
        } else if (!growing && required < w) {
          required = w;
        }

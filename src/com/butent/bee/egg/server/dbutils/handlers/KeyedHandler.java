 ...
 */
package com.butent.bee.egg.server.dbutils.handlers;

import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.Map;

import com.butent.bee.egg.server.dbutils.RowProcessor;

/**
 * <p>
 * <code>ResultSetHandler</code> implementation that returns a Map of Maps.
 * <code>ResultSet</code> rows are converted into Maps which are then stored in
...
public class KeyedHandler extends
    AbstractKeyedHandler<Object, Map<String, Object>> {
  ...
  /**
   * Creates a new instance of KeyedHandler. The value of the first column of
   * each row will be a key in the Map.
   */
  public KeyedHandler() {
   ...
   * 
   * @param columnIndex
   *          The values to use as keys in the Map are retrieved from the column
   *          at this index.
   */
  ...
  /**
   * This factory method is called by <code>handle()</code> to retrieve the key
   * value from the current <code>ResultSet</code> row. This implementation
   * returns <code>ResultSet.getObject()</code> for the configured key column
   * name or index.
   * 
   * @param rs
   ...
   * <code>ResultSet</code> row in some object. This implementation returns a
   * <code>Map</code> with case insensitive column names as keys. Calls to
   * <code>map.get("COL")</code> and <code>map.get("col")</code> return the same
   * value.
   * 
   * @param rs

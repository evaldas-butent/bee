 ...
 */
package com.butent.bee.egg.server.lang.reflect;
...
import com.butent.bee.egg.server.lang.ClassUtils;
 ...
 * @author James Carman
 * @since 3.0
 * @version $Id: TypeUtils.java 966171 2010-07-21 10:47:57Z sebb $
 */
public class TypeUtils {
   ...
   */
  private static boolean isAssignable(Type type,
      TypeVariable<?> toTypeVariable, Map<TypeVariable<?>, Type> typeVarAssigns) {
    if (type == null) {
      return true;
    }
   ...
   * 
   * @param type
   *          specifies the subject parameterized type from which to harvest the
   *          parameters.
   ...
   * assignable to <code>toClass</code>. It returns an empty map if none of the
   * classes or interfaces in its inheritance hierarchy specify any type
   * arguments. </p>
   * <p>
   ...
   * <code>type</code> and <code>toClass</code>. So with the above example, this
   * method will also determine that the type arguments for
   * {@link java.util.Hashtable Hashtable} are also both <code>Object</code>. In
   * cases where the interface specified by <code>toClass</code> is (indirectly)
   * implemented more than once (e.g. where <code>toClass</code> specifies the
   * interface {@link java.lang.Iterable Iterable} and <code>type</code>
   * specifies a parameterized type that implements both {@link java.util.Set
   * Set} and {@link java.util.Collection Collection}), this method will look at
   * the inheritance hierarchy of only one of the implementations/subclasses;
   * the first interface encountered that isn't a subinterface to one of the
   * others in the <code>type</code> to <code>toClass</code> hierarchy.
   * </p>
   * 
   * @param type
   ...
   *          <code>toClass</code>
   * @param toClass
   *          the class whose type parameters are to be determined based on the
  ...
  private static Map<TypeVariable<?>, Type> getTypeArguments(
    ...
    for (int i = 0; i < typeParams.length; i++) {
      ...
      typeVarAssigns.put(typeParams[i],
          typeVarAssigns.containsKey(typeArg) ? typeVarAssigns.get(typeArg)
              : typeArg);
    }
   ...
   * arguments based on a subtype. It is far more limited in determining the
   * type arguments for the subject class's type variables in that it can only
   ...
   * @param superType
   *          the super type from which <code>cls</code>'s type arguments are to
   *          be determined
   * @return a map of the type assignments that could be determined for the type
   *         variables in each type in the inheritance hierarchy from
  ...
  /**
   * <p>
   * Closest parent type? Closest to what? The closest parent type to the super
   * class specified by <code>superClass</code>.
   * </p>
   * 
   * @param cls
   * @param superClass
   ...
   * types and wildcard types (or it would with wildcard types if multiple upper
   * bounds were allowed).
   * </p>
   * <p>
   ...
   * </pre>
   * 
   * since <code>List</code> is a subinterface of <code>Collection</code>, this
   * method will return the bounds as if the declaration had been:
   * 
   * <pre>
   ...
   * returns the result of <code>WildcardType.getUpperBounds()</code> passed
   ...
   * returns the result of <code>WildcardType.getLowerBounds()</code>.
   * </p>
   * 
   * @param wildcardType
   *          the subject wildcard type
   ...
   * <code><T, S extends Comparable<T></code>), or otherwise depends on another
   * type variable to be specified, the dependencies must be included in
   ...
   * {@link TypeVariable}s and {@link GenericArrayType}s, or when you do not
   * know the runtime type of <code>type</code>: if you know you have a
   ...
   * @return the resolved <code>Class</code> object or <code>null</code> if the
  ...
  public static Class<?> getRawType(Type type, Type assigningType) {
    ...
    if (type instanceof GenericArrayType) {
      ...
      Class<?> rawComponentType = getRawType(
          ((GenericArrayType) type).getGenericComponentType(), assigningType);

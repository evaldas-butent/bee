 ...
 * @since 2.2
 * @version $Id: LocaleUtils.java 889215 2009-12-10 11:56:38Z bayard $
 */
public class LocaleUtils {
   ...
   * availableLocaleList() to avoid the synchronized block affecting normal use,
   * yet synchronized and lazy loading to avoid a static block affecting other
   ...
   * availableLocaleSet() to avoid the synchronized block affecting normal use,
   * yet synchronized and lazy loading to avoid a static block affecting other
   * methods in this class.
   */
  private static synchronized void initAvailableLocaleSet() {
    if (cAvailableLocaleSet == null) {
      cAvailableLocaleSet = Collections.unmodifiableSet(new HashSet<Locale>(
          availableLocaleList()));
    }
  }

...
public abstract class StrMatcher {
   ...
   */
  private static final StrMatcher QUOTE_MATCHER = new CharSetMatcher(
      "'\"".toCharArray());
  /**
   ...
   * <p>
   * This method is called to check for a match. The parameter <code>pos</code>
   * represents the current position to be checked in the string
   * <code>buffer</code> (a character array which must not be changed). The API
   * guarantees that <code>pos</code> is a valid index for <code>buffer</code>.
   * <p>
   * The character array may be larger than the active area to be matched. Only
   * values in the buffer between the specifed indices may be accessed.
   * <p>
   * The matching code may check one character or many. It may check characters
   * preceeding <code>pos</code> as well as those after, so long as no checks
   ...
   */
  public abstract int isMatch(char[] buffer, int pos, int bufferStart,
      int bufferEnd);
   ...
   * <p>
   * The matching code may check one character or many. It may check characters
   * preceeding <code>pos</code> as well as those after.
   * <p>
   * It must return zero for no match, or a positive number if a match was
   * found. The number indicates the number of characters that matched.
   * 

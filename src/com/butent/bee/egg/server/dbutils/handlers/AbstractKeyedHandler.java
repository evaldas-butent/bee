...
import com.butent.bee.egg.server.dbutils.ResultSetHandler;
 ...
 * stored in a Map under the given keys (Ks).
 * </p>
 * 
...
public abstract class AbstractKeyedHandler<K, V> implements
  ...
  /**
   * Convert each row's columns into a Map and store then in a <code>Map</code>
   * under <code>ResultSet.getObject(key)</code> key.
   * 
   * @param rs
  ...
  /**
   * This factory method is called by <code>handle()</code> to create the Map to
   * store records in. This implementation returns a <code>HashMap</code>
   * instance.
   * 
   ...
   * value from the current <code>ResultSet</code> row.
   * 
   * @param rs
   ...
   * <code>ResultSet</code> row in some object.
   * 
   * @param rs

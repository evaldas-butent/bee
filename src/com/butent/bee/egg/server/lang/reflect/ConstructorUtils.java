...
public class ConstructorUtils {
   ...
   * {@link #invokeExactConstructor(Class, Object[], Class[])} for more details.
   * </p>
   * 
   * <p>
   ...
   * 
   * <p>
   * First it checks if there is constructor matching the exact signature. If no
   * such, all the constructors of the class are tested if their signatures are
   * assignment compatible with the parameter types. The first matching
  ...
  public static <T> Constructor<T> getMatchingAccessibleConstructor(
    ...
    for (int i = 0; i < ctors.length; i++) {
      // compare parameters
      if (ClassUtils.isAssignable(parameterTypes, ctors[i].getParameterTypes(),
          true)) {

...
public class Popup extends Simple implements HasAnimation, CloseEvent.HasCloseHandlers,
  ...
  protected boolean isViewHeaderEvent(NativeEvent event) {
  ...
  protected void setDimensions(HasDimensions dimensions, double maxWidthFactor,
    ...
    if (v > 0) {
      if (u == CssUnit.PX) {
        v = Math.round(Math.min(v, Window.getClientWidth() * maxWidthFactor));
      }
    ...
    if (v > 0) {
      if (u == CssUnit.PX) {
        v = Math.round(Math.min(v, Window.getClientHeight() * maxHeightFactor));
      }

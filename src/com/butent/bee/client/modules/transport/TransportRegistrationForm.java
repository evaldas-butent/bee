...
class TransportRegistrationForm extends AbstractFormInterceptor {
  ...
  private void onCreateUser() {
    ...
    parameters.put(ClassifiersConstants.COL_EMAIL, email);

    putUserField(parameters, COL_REGISTRATION_COMPANY_NAME, ClassifiersConstants.ALS_COMPANY_NAME);
    putUserField(parameters, COL_REGISTRATION_COMPANY_CODE, ClassifiersConstants.ALS_COMPANY_CODE);
    putUserField(parameters, COL_REGISTRATION_VAT_CODE, ClassifiersConstants.COL_COMPANY_VAT_CODE);
    putUserField(parameters, COL_REGISTRATION_EXCHANGE_CODE,
        ClassifiersConstants.COL_COMPANY_EXCHANGE_CODE);
    ...
    if (!BeeUtils.isEmpty(contact)) {
      ...
      if (p > 0) {
        parameters.put(ClassifiersConstants.COL_FIRST_NAME, contact.substring(0, p).trim());
        parameters.put(ClassifiersConstants.COL_LAST_NAME, contact.substring(p + 1).trim());
      } else {
        parameters.put(ClassifiersConstants.COL_FIRST_NAME, contact);
      }
    }
    putUserField(parameters, COL_REGISTRATION_CONTACT_POSITION, ClassifiersConstants.COL_POSITION);

    putUserField(parameters, COL_REGISTRATION_ADDRESS, ClassifiersConstants.COL_ADDRESS);
    putUserField(parameters, COL_REGISTRATION_CITY, ClassifiersConstants.COL_CITY);
    putUserField(parameters, COL_REGISTRATION_COUNTRY, ClassifiersConstants.COL_COUNTRY);

    putUserField(parameters, COL_REGISTRATION_PHONE, ClassifiersConstants.COL_PHONE);
    putUserField(parameters, COL_REGISTRATION_MOBILE, ClassifiersConstants.COL_MOBILE);
    putUserField(parameters, COL_REGISTRATION_FAX, ClassifiersConstants.COL_FAX);

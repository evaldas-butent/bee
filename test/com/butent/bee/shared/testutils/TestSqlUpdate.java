...
public class TestSqlUpdate {
  ...
  public final void testGetSources() {
    ...
    assertEquals(
        "UPDATE Source1 alias1 SET name=? FROM Source5, Source4, Source3 WHERE Source2.name=?",
        update2.getSqlString(builder, true));
  }
  ...
  public final void testGetSqlString() {
    ...
    select.setWhere(SqlUtils.contains(SqlUtils.expression(SqlUtils.name("field2")),
        "naikinamas_irasas"));
    update3.addFrom(select, "selAlias");

    assertEquals(
        "UPDATE Source1 alias1 SET name=? FROM Source5, Source4, Source3, (SELECT Table1.field1, Table1.field2 FROM InFrom WHERE field2 LIKE ?) selAlias WHERE Source2.name=?",
        update3.getSqlString(builder, true));

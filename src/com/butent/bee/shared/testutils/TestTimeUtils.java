...
public class TestTimeUtils {
	...
	public final void testRandomDate() {
		...
		for (int i = 0; i < this.MAX_RANDOM_REQUEST; i++) {
	...
	public final void testFieldName() {
		...
		try {
			...
			for (int i = 0; i < m.length; i++) {
				...
				if (m[i].toString()
						.equals("private static java.lang.String com.butent.bee.shared.utils.TimeUtils.fieldName(int)")) {
					index = i;
				}
			}
		} catch (Throwable e) {
		...
		try {
			...
			assertEquals("Field -5", meth.invoke(null, arglist));

			arglist[0] = 0;
			assertEquals("ERA", meth.invoke(null, arglist));

			arglist[0] = 100;
			assertEquals("Field 100", meth.invoke(null, arglist));

			arglist[0] = 5;
			assertEquals("DAY_OF_MONTH", meth.invoke(null, arglist));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

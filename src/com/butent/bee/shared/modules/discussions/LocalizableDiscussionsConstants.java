package com.butent.bee.shared.modules.discussions;

public interface LocalizableDiscussionsConstants {

  String announcement();

  String announcementNew();

  String announcements();

  String adTopic();

  String birthdaysParties();

  String discussion();

  String discussions();

  String discussActionActivate();

  String discussActionClose();

  String discussActionComment();

  String discussActionDeactivate();

  String discussActionMark();

  String discussActionModify();

  String discussActionReply();

  String discussActivationQuestion();

  String discussActive();

  String discussAll();

  String discussAllShort();

  String discussOwner();

  String discussClosed();

  String discussCloseQuestion();

  String discussComment();

  String discussCommentCount();

  String discussCommentPlaceholder();

  String discussCommentPermit();

  String discussCreatedNewDiscussion();

  String discussDeleteCanOwnerOrAdmin();

  String discussDeleteQuestion();

  String discussDescription();

  String discussEnterComment();

  String discussEventActivated();

  String discussEventClosed();

  String discussEventCommented();

  String discussEventCommentDeleted();

  String discussEventCreated();

  String discussEventDeactivated();

  String discussEventMarked();

  String discussEventModified();

  String discussEventReplied();

  String discussEventVisited();

  String discussFiles();

  String discussFilterStarred();

  String discussInvalidFile();

  String discussObserved();

  String discussMailNewAnnouncementSubject();

  String discussMailNewDiscussionSubject();

  String discussMarks();

  String discussMarked();

  String discussMarkStats();

  String discussMembers();

  String discussNotCreated();

  String discussPublic1();

  String discussPrivateShort();

  String discussSelectMembers();

  String discussStatusActive();

  String discussStatusClosed();

  String discussStatusCommented();

  String discussStatusInactive();

  String discussStatusNew();

  String discussStarred();

  String displayInBoard();

  String more();
}

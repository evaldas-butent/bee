...
package com.butent.bee.egg.server.lang.event;
...
import com.butent.bee.egg.server.lang.Validate;
...
public class EventListenerSupport<L> {
  /**
   * The list used to hold the registered listeners. This list is intentionally
   * a thread-safe copy-on-write-array so that traversals over the list of
   * listeners will be atomic.
   */
  ...
  /**
   * Creates an EventListenerSupport object which supports the provided listener
   * interface.
   * 
   * @param listenerInterface
   *          the type of listener interface that will receive events posted
  ...
  /**
   * Creates an EventListenerSupport object which supports the provided listener
   * interface using the specified class loader to create the JDK dynamic proxy.
   * 
   * @param listenerInterface
   ...
   * 
   * @throws NullPointerException
   *           if <code>listenerInterface</code> or <code>classLoader</code> is
   *           <code>null</code>.
   * @throws IllegalArgumentException
   *           if <code>listenerInterface</code> is not an interface.
   */
  public EventListenerSupport(Class<L> listenerInterface,
    ...
    proxy = listenerInterface.cast(Proxy.newProxyInstance(classLoader,
        new Class[] { listenerInterface }, new ProxyInvocationHandler()));
  }

  /**
   * Returns a proxy object which can be used to call listener methods on all of
   * the registered event listeners. All calls made to this proxy will be
   * forwarded to all registered listeners.
   * 
   * @return a proxy object which can be used to call listener methods on all of
  ...
  private class ProxyInvocationHandler implements InvocationHandler {
    /**
     * Propagates the method call to all registered listeners in place of the
     * proxy listener object.
     * 
     * @param proxy
     ...
     * @param args
     *          event arguments to propogate to the listeners.
     */

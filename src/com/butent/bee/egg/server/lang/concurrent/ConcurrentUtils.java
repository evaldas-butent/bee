 ...
 * @version $Id: ConcurrentUtils.java 929189 2010-03-30 16:49:22Z oheger $
 */
public class ConcurrentUtils {
  ...
  /**
   * Inspects the cause of the specified {@code ExecutionException} and creates
   * a {@code ConcurrentException} with the checked cause if necessary. This
   * method performs the following checks on the cause of the passed in
   * exception:
   * <ul>
   * <li>If the passed in exception is <b>null</b> or the cause is <b>null</b>,
   * this method returns <b>null</b>.</li>
  ...
  /**
   * Inspects the cause of the specified {@code ExecutionException} and creates
   * a {@code ConcurrentRuntimeException} with the checked cause if necessary.
   * This method works exactly like {@link #extractCause(ExecutionException)}.
   * The only difference is that the cause of the specified
   * {@code ExecutionException} is extracted as a runtime exception. This is an
   * alternative for client code that does not want to deal with checked
   * exceptions.
   * 
   * @param ex
   *          the exception to be processed
   ...
   * being thrown. If the cause is a checked exception however, it is wrapped in
   * a {@code ConcurrentException}, which is thrown. If the passed in exception
   * is <b>null</b> or has no cause, the method simply returns without throwing
   * an exception.
   * 
   * @param ex
   *          the exception to be handled
   * @throws ConcurrentException
   *           if the cause of the {@code ExecutionException} is a checked
   *           exception
   */
   ...
   * {@link ConcurrentException} it throws a {@link ConcurrentRuntimeException}.
   * This is an alternative for client code that does not want to deal with
   * checked exceptions.
   * 
   * @param ex
   *          the exception to be handled
   * @throws ConcurrentRuntimeException
   *           if the cause of the {@code ExecutionException} is a checked
   *           exception; this exception is then wrapped in the thrown runtime
   *           exception
   */
   ...
   * 
   * @param ex
   *          the {@code Throwable} to check
   * @return a flag whether the passed in exception is a checked exception
   * @throws IllegalArgumentException
   *           if the {@code Throwable} is not a checked exception
   */
  ...
  /**
   * Tests whether the cause of the specified {@code ExecutionException} should
   * be thrown and does it if necessary.
   * 
   * @param ex
   ...
   * produced by the initializer. This method just invokes the {@code get()}
   * method of the given {@code ConcurrentInitializer}. It is <b>null</b>-safe:
   * if the argument is <b>null</b>, result is also <b>null</b>.
   * 
   * @param <T>
   ...
   * @throws ConcurrentException
   *           if the {@code ConcurrentInitializer} throws an exception
   */
   ...
   * {@link #initialize(ConcurrentInitializer)}, but if the
   * {@code ConcurrentInitializer} throws a {@link ConcurrentException}, it is
  ...
  /**
   * <p>
   * Gets an implementation of <code>Future</code> that is immediately done and
   * returns the specified constant value.
   * </p>
   * <p>
   * This can be useful to return a simple constant immediately from the
   * concurrent processing, perhaps as part of avoiding nulls. A constant future
   * can also be useful in testing.
   * </p>
   * 
   * @param value
   *          the constant value to return, may be null

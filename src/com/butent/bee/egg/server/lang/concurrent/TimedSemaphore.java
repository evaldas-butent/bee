 ...
 * However, there is an additional timing dimension: there is no
 * {@code release()} method for freeing a permit, but all permits are
 * automatically released at the end of a configurable time frame. If a thread
 * calls {@link #acquire()} and the available permits are already exhausted for
 * this time frame, the thread is blocked. When the time frame ends all permits
 ...
 * corresponding period to monitor this interval. The
 * {@code ScheduledExecutorService} to be used for this purpose can be provided
 * at construction time. Alternatively the class creates an internal executor
 * service.
 * </p>
 * <p>
...
public class TimedSemaphore {
  /**
   * Constant for a value representing no limit. If the limit is set to a value
   * less or equal this constant, the {@code TimedSemaphore} will be effectively
   * switched off.
   */
  ...
  /**
   * The counter for the periods. This counter is increased every time a period
   * ends.
   */
  ...
  /**
   * Creates a new instance of {@link TimedSemaphore} and initializes it with an
   * executor service, the given time period, and the limit. The executor
   ...
   * invocations of {@link #acquire()} are allowed within the monitored period.
   * 
   * @return the limit
   */
   ...
   * further invocations of {@link #acquire()} will block. Setting the limit to
   * a value &lt;= {@link #NO_LIMIT} will cause the limit to be disabled, i.e.
   * an arbitrary number of{@link #acquire()} invocations is allowed in the time
   * period.
   * 
   * @param limit
   *          the limit
  ...
  /**
   * Initializes a shutdown. After that the object cannot be used any more. This
   * method can be invoked an arbitrary number of times. All invocations after
   * the first one do not have any effect.
   */
  ...
  /**
   * Returns the number of invocations of the {@link #acquire()} method for the
   * current period. This may be useful for testing or debugging purposes.
   * 
  ...
  /**
   * Returns the number of calls to the {@link #acquire()} method that can still
   * be performed in the current period without blocking. This method can give
   * an indication whether it is safe to call the {@link #acquire()} method
   * without risking to be suspended. However, there is no guarantee that a
   * subsequent call to {@link #acquire()} actually is not-blocking because in
   * the mean time other threads may have invoked the semaphore.
   * 
  ...
  /**
   * Returns the time period. This is the time monitored by this semaphore. Only
   * a given number of invocations of the {@link #acquire()} method is possible
   * in this period.
   * 

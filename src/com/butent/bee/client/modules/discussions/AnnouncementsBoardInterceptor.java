...
class AnnouncementsBoardInterceptor extends AbstractFormInterceptor implements
  ...
  private class AnnouncmentTopicWidget extends Flow {
    ...
    private final Button moreButton = new Button("caption");
    ...
    AnnouncmentTopicWidget(final Long discussId) {
      ...
      moreButton.addStyleName(STYLE_ACTION + COL_DISCUSSION);
      // eventContainer.add(moreButton);
      ...
      Image currUserPhoto = new Image(photoFileName);
      if (!BeeUtils.isEmpty(photoFileName)) {
        currUserPhoto.setUrl("images/photo/" + photoFileName);
      } else {
      ...
      sendLabel.setTitle(Localized.getConstants().crmActionComment());
    ...
    void setEnableCommenting(boolean enabled) {
      ...
      if (enabled) {
        ...
        DomUtils.setPlaceholder(commentInput, Localized.getConstants().discussCommentPlaceholder());
        sendLabel.setEnabled(true);
        att.setVisible(true);
      } else {
        commentInput.setEnabled(false);
        DomUtils.setPlaceholder(commentInput, Localized.getConstants().discussCommentPermit());
  ...
  protected void renderAnnoucementsSection(final SimpleRow rsRow,
    ...
    if (DataUtils.isId(rowId)) {
      tWidget.openOnClick(VIEW_DISCUSSIONS, rowId);
    }
    ...
    String photoUrl = PhotoRenderer.getUrl(rsRow.getValue(COL_PHOTO));

    if (!BeeUtils.isEmpty(rsRow.getValue(COL_PHOTO))) {
      tWidget.setPhoto(photoUrl);
    } else {
    ...
    tWidget.setTopicColor(rsRow.getValue(COL_BG_COLOR), rsRow.getValue(COL_TXT_COLOR));
    ...
    if (DataUtils.isId(rsRow.getLong(COL_DISCUSSION_COM_ID))) {
      tWidget.setCommentCount(BeeUtils.joinWords(rsRow.getValue(COL_DISCUSSION_COM_ID),
          Localized.getConstants().discussCommentCount()));
    } else {
  ...
  protected void renderWelcomeSection(Flow adsTable) {
    ...
    welcome.setSummary(Localized.getConstants().welcomeMessage());
  ...
  protected void renderBirthdaySection(SimpleRow rsRow,
    ...
    birthTable.setHtml(row, 0, Localized.getConstants().birthdaysParties(), STYLE_PREFIX
        + COL_SUBJECT);
    birthTable.getRow(row).addClassName(STYLE_PREFIX + ALS_BIRTHDAY);

    Flow topicName = new Flow();
    birthTable.setWidget(row, 1, topicName);
    ...
    StyleUtils.setBackgroundColor(topicN.getElement(), rsRow.getValue(ALS_BG_COLOR));
    StyleUtils.setColor(topicN.getElement(), rsRow.getValue(ALS_TXT_COLOR));
  ...
  private void clearUnused(Flow panel, SimpleRowSet data) {
    ...
    for (Long id : cachedContainers.keySet()) {
      if (!DataUtils.isId(data.getValueByKey(COL_DISCUSION, BeeUtils.toString(id),
          COL_DISCUSION))) {
  ...
  private void renderBirthdaysList(final int contentRow, final HtmlTable birthContent,
    ...
    for (String[] birthListData : rs.getRows()) {
      ...
      String photoUrl1;
      if (!BeeUtils.isEmpty(photo1)) {
        photoUrl1 =
            PhotoRenderer.getUrl(photo1);
      } else {

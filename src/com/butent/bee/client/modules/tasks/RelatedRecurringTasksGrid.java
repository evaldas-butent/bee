...
class RelatedRecurringTasksGrid extends AbstractGridInterceptor {
    ...
    if (copy) {
      ...
      Global.confirm(caption, Icon.QUESTION, messages, Localized.getConstants().actionCopy(),
          Localized.getConstants().actionCancel(), new ConfirmationCallback() {
            ...
            public void onConfirm() {
              ...
              Long rtId = presenter.getActiveRow().getLong(getDataIndex(COL_RECURRING_TASK));
      ...
      presenter.getGridView().ensureRelId(new IdCallback() {
        ...
        public void onSuccess(Long relId) {
          ...
          RowActionEvent.fireCreateRow(VIEW_RECURRING_TASKS, row, relColumn);

          String relation = Data.getColumnRelation(AdministrationConstants.VIEW_RELATIONS,
              relColumn);
          if (!BeeUtils.isEmpty(relation)) {
            row.setProperty(relation, relId.toString());
          }
  ...
  public void onEditStart(EditStartEvent event) {
    ...
    int index = getDataIndex(COL_RECURRING_TASK);
    if (!BeeConst.isUndef(index) && event.getRowValue() != null) {
      openRecurringTask(event.getRowValue().getLong(index));
    }
  }

 ...
 * {@code AtomicInitializer} object before it has been initialized almost at the
 * same time, it can happen that {@link #initialize()} is called multiple times.
 * The algorithm outlined above guarantees that {@link #get()} always returns
...
public abstract class AtomicInitializer<T> implements ConcurrentInitializer<T> {
  ...
  /**
   * Returns the object managed by this initializer. The object is created if it
   * is not available yet and stored internally. This method always returns the
   * same object.
   * 
   ...
   * {@code AtomicInitializer}. This method is called by {@link #get()} when the

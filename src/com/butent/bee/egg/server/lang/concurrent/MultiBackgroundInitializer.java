 ...
 */
package com.butent.bee.egg.server.lang.concurrent;
 ...
 * initialization tasks to be performed and add them to the
 * {@code MultiBackgroundInitializer} using the
 ...
 * </li>
 * <li>When access to the result objects produced by the
 * {@code BackgroundInitializer} objects is needed call the {@link #get()}
 * method. The object returned here provides access to all result objects
 * created during initialization. It also stores information about exceptions
 * that have occurred.</li>
 * </ul>
 * </p>
 * <p>
 ...
 * already has an {@code ExecutorService} set. If this is the case, this
 * {@code ExecutorService} is used for running the background task. Otherwise
 * the current {@code ExecutorService} of this
 * {@code MultiBackgroundInitializer} is shared with the initializer.
 * </p>
 * <p>
 ...
 * <strong>Important note:</strong> When using an external
 * {@code ExecutorService} be sure that the number of threads managed by the
 * service is large enough. Otherwise a deadlock can happen! This is the case in
 * the following scenario: {@code MultiBackgroundInitializer} starts a task that
 ...
 * </p>
 * 
 * @version $Id: MultiBackgroundInitializer.java 889215 2009-12-10 11:56:38Z
 *          bayard $
 */
public class MultiBackgroundInitializer
   ...
   * @param init
   *          the {@code BackgroundInitializer} to add (must not be <b>null</b>)
   ...
   * {@code BackgroundInitializer} objects in parallel. This implementation sums
   * up the required tasks for all child initializers (which is necessary if one
   * of the child initializers is itself a {@code MultiBackgroundInitializer} ).
   * Then it adds 1 for the control task that waits for the completion of the
   * children.
   * 
   ...
   * {@code BackgroundInitializer} objects. Then it collects their results and
   * creates a {@code MultiBackgroundInitializerResults} object with this data.
   * If a child initializer throws a checked exceptions, it is added to the
   ...
   * class are returned by {@link MultiBackgroundInitializer#initialize()}. They
   * allow access to all result objects produced by the
   * {@link BackgroundInitializer} objects managed by the owning instance. It is
   * also possible to retrieve status information about single
  ...
  public static class MultiBackgroundInitializerResults {
    ...
    /**
     * Creates a new instance of {@code MultiBackgroundInitializerResults} and
     * initializes it with maps for the {@code BackgroundInitializer} objects,
    ...
    private MultiBackgroundInitializerResults(
        Map<String, BackgroundInitializer<?>> inits,
        Map<String, Object> results, Map<String, ConcurrentException> excepts) {
      initializers = inits;
    ...
    /**
     * Returns the result object produced by the {@code BackgroundInitializer}
     * with the given name. This is the object returned by the initializer's
     * {@code initialize()} method. If this {@code BackgroundInitializer} caused
     * an exception, <b>null</b> is returned. If the name cannot be resolved, an
     * exception is thrown.
     * 
     * @param name
     *          the name of the {@code BackgroundInitializer}
    ...
    /**
     * Returns a flag whether the {@code BackgroundInitializer} with the given
     ...
     * {@code BackgroundInitializer} with the given name. If this initializer
     * did not throw an exception, the return value is <b>null</b>. If the name
    ...
    /**
     * Returns a set with the names of all {@code BackgroundInitializer} objects
     * managed by the {@code MultiBackgroundInitializer}.
     * 
     * @return an (unmodifiable) set with the names of the managed
     *         {@code BackgroundInitializer} objects
     */
    ...
    /**
     * Checks whether an initializer with the given name exists. If not, throws
     * an exception. If it exists, the associated child initializer is returned.
     * 
     * @param name
     *          the name to check
    ...
    private BackgroundInitializer<?> checkName(String name) {
      ...
      if (init == null) {
        throw new NoSuchElementException("No child initializer with name "
            + name);
      }

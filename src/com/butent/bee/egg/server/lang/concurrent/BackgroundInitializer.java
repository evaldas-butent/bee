 ...
 * task. If no {@code ExecutorService} has been provided,
 * {@code BackgroundInitializer} creates a temporary {@code ExecutorService} and
...
public abstract class BackgroundInitializer<T> implements
  ...
  /**
   * Creates a new instance of {@code BackgroundInitializer} and initializes it
   * with the given {@code ExecutorService}. If the {@code ExecutorService} is
   * not null, the background task for initializing this object will be
   * scheduled at this service. Otherwise a new temporary
   * {@code ExecutorService} is created.
   * 
   * @param exec
   *          an external {@code ExecutorService} to be used for task execution
   */
  ...
  /**
   * Returns a flag whether this {@code BackgroundInitializer} has already been
   * started.
   * 
   * @return a flag whether the {@link #start()} method has already been called
   */
  ...
  /**
   * Sets an {@code ExecutorService} to be used by this class. The
   * {@code ExecutorService} passed to this method is used for executing the
   ...
   * becomes active and invokes the {@link #initialize()} method in a background
   * task. A {@code BackgroundInitializer} can be started exactly once. The
   * return value of this method determines whether the start was successful:
   * only the first invocation of this method returns <b>true</b>, following
   * invocations will return <b>false</b>.
   * 
   ...
   * was called causes an {@code IllegalStateException} exception to be thrown.
   * 
  ...
  /**
   * Returns the {@code ExecutorService} that is actually used for executing the
   * background task. This method can be called after {@link #start()} (before
   * {@code start()} it returns <b>null</b>). If an external executor was set,
   * this is also the active executor. Otherwise this method returns the
   * temporary executor that was created by this object.
   * 
   ...
   * This information is evaluated when a temporary {@code ExecutorService} is
   * created. This base implementation returns 1. Derived classes that do more
   * complex background processing can override it. This method is called from a
   * synchronized block by the {@link #start()} method. Therefore overriding
   * methods should be careful with obtaining other locks and return as fast as
   * possible.
   * 
   ...
   * when this {@code BackgroundInitializer} is started. It must be implemented
   * by a concrete subclass. An implementation is free to perform arbitrary
   * initialization. The object returned by this method can be queried using the
   * {@link #get()} method.
   * 
   ...
   * method. If a temporary {@code ExecutorService} is used, it is destroyed at
   * the end of the task.
   * 
   * @param execDestroy
  ...
  /**
   * Creates the {@code ExecutorService} to be used. This method is called if no
   * {@code ExecutorService} was provided at construction time.
   * 
  ...
  private class InitializationTask implements Callable<T> {
    ...
    /**
     * Creates a new instance of {@code InitializationTask} and initializes it
     * with the {@code ExecutorService} to be destroyed at the end.
     * 
     * @param exec

...
public class OrderForm extends PrintFormInterceptor {
  ...
  public void afterRefresh(final FormView form, final IsRow row) {
    ...
    String msg = Objects.equals(getStatus(), OrdersStatus.NEW.ordinal()) ? loc.ordAskEcApprove()
        : loc.ordAskApprove();

    Button approve = new Button(loc.ordApprove(), event -> Global.confirm(msg, () -> {
      ...
      Long series = row.getLong(getDataIndex(COL_SERIES));
      if (!DataUtils.isId(series)) {
        form.notifySevere(Localized.dictionary().trdSeries() + " "
            + Localized.dictionary().valueRequired());
        return;
      }
  ...
  private String getFileName() {
    ...
    String invoicePrefix =
        row.getString(Data.getColumnIndex(VIEW_ORDERS, COL_SERIES_NAME));
    String number =

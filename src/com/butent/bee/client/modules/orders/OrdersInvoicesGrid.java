    ...
    if (Objects.equals(event.getSource(), joinAction)) {
      ...
      Queries.getRowSet(getViewName(), null, Filter.idIn(ids), result -> {
        ...
        for (String col : new String[] {
            COL_TRADE_EXPORTED, COL_TRADE_CUSTOMER,
        ...
        RowFactory.createRow(dataInfo.getNewRowForm(), dataInfo.getNewRowCaption(), dataInfo,
            newRow, Opener.MODAL, new NewOrderInvoiceForm(),
            newInvoice -> Queries.update(VIEW_SALE_ITEMS, Filter.any(COL_SALE, ids), COL_SALE,
                BeeUtils.toString(newInvoice.getId()),
                res1 -> Queries.delete(VIEW_SALES, Filter.idIn(ids),
                    result1 -> Data.onViewChange(VIEW_ORDER_CHILD_INVOICES,
